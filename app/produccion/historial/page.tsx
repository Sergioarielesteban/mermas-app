...
import {
  ...
  fetchChefProductionSessions,
  fetchChefProductionTemplatesByIds,
} from '@/lib/chef-ops-supabase';

export default function ProduccionHistorialPage() {
  const { localId, profileReady } = useAuth();
  const supabaseOk = isSupabaseEnabled() && getSupabaseClient();
  const [sessions, setSessions] = useState<ChefProductionSession[]>([]);
  const [metaById, setMetaById] = useState<Record<string, ChefProductionTemplate>>({});
  ...
  const load = useCallback(async () => {
    ...
    setBanner(null);
    try {
      const rs = await fetchChefProductionSessions(supabase, localId, 60);
      setSessions(rs);
      const ids = rs.map((r) => r.templateId);
      const metas = await fetchChefProductionTemplatesByIds(supabase, localId, ids);
    ...
    <div className="space-y-4 pb-10">
      ...
      {!localId || !supabaseOk ? (
        ...
        <p className="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-8 text-center text-sm text-zinc-600">
          Aún no hay listas de producción.
        </p>
      ) : (
        <ul className="space-y-2">
          {rows.map((r) => {
            const m = metaById[r.templateId];
            const title = m?.name ?? 'Plantilla (desconocida o eliminada)';
