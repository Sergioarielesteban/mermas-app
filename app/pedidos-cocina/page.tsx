...
export default function PedidosCocinaPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return catalog;
    return catalog.filter((r) => r.product_name.toLowerCase().includes(q));
  }, [catalog, query]);

  const cartLines = useMemo(() => {
    const lines: Array<{ row: CentralSupplyCatalogRow; qty: number; line: number }> = [];
    for (const row of catalog) {
      const qty = qtyByProduct[row.product_id] ?? 0;
      if (qty > 0) {
        const line = Math.round(qty * Number(row.price_per_unit) * 100) / 100;
        lines.push({ row, qty, line });
      }
    }
    return lines.sort((a, b) => a.row.product_name.localeCompare(b.row.product_name, 'es'));
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Precios del catálogo central. El total se confirma al enviar (recalculado en servidor).
          </p>
        </div>
        <Link
      ...
      {loading ? (
        ...
        <ul className="space-y-2">
          {filtered.map((row) => {
            const q = qtyByProduct[row.product_id] ?? 0;
            return (
              <li
                key={row.product_id}
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="font-bold text-zinc-900">{row.product_name}</p>
                  <p className="mt-0.5 text-xs font-medium text-zinc-600">
                    {formatEur(Number(row.price_per_unit))} / {CC_UNIT_SHORT[row.unit as keyof typeof CC_UNIT_SHORT] ?? row.unit}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setQty(row.product_id, q - 1)}
          ...
          <div>
            ...
            {cartLines.length > 0 ? (
              <p className="mt-1 line-clamp-2 text-xs text-zinc-600">
                {cartLines.map((l) => `${l.qty}× ${l.row.product_name}`).join(' · ')}
              </p>
            ) : (
