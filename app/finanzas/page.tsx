    ...
    <div className="space-y-4 pb-10">
      ...
      <section className={`rounded-2xl p-4 ring-2 ${hs.ring} ${hs.bg} sm:p-5`}>
        ...
        {data ? (
          <button
            ...
            className="mt-3 flex w-full min-h-[44px] items-center justify-center gap-2 rounded-xl border border-zinc-200/80 bg-white/60 text-xs font-bold text-zinc-700 sm:w-auto sm:px-4"
          >
            {healthOpen ? 'Ocultar' : 'Ver'} detalle de señales
            <ChevronDown className={`h-4 w-4 transition ${healthOpen ? 'rotate-180' : ''}`} aria-hidden />
          </button>
        ) : null}
        {healthOpen && data ? (
          <div className="mt-2 space-y-4">
            <ul className="list-disc space-y-1 pl-5 text-sm text-zinc-700">
              {data.healthReasons.map((r, i) => (
                <li key={i}>{r}</li>
              ))}
            </ul>
            <div className="rounded-xl border border-zinc-200/90 bg-white/70 p-3 text-xs leading-relaxed text-zinc-600">
              <p className="font-bold text-zinc-800">Criterios aplicados (fase umbrales)</p>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-200">
          ...
          <p className="text-[10px] text-zinc-500">Neto (sin IVA)</p>
        ...
        <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-200">
          ...
          <p className="text-[10px] text-zinc-500">Neto pedidos enviados − neto albaranes</p>
        </div>
        <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-200">
            ...
            <div className="mt-4 h-56 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topSuppliersChart} layout="vertical" margin={{ top: 8, right: 16, left: 8, bottom: 8 }}>
                  ...
                  <Bar dataKey="net" fill="#D32F2F" radius={[0, 4, 4, 0]} name="Gasto neto" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
              ...
              <tbody>
                {data.topSuppliers.slice(0, 5).map((r) => (
                  <tr key={r.supplierName + r.net} className="border-t border-zinc-100">
                    <td className="py-2 pr-2 font-semibold text-zinc-900">{r.supplierName}</td>
            ...
            <table className="min-w-full text-left text-xs sm:text-sm">
              ...
              <tbody>
                {data.topMermas.slice(0, 5).map((r) => (
                  <tr key={r.key} className="border-t border-zinc-100">
                    <td className="py-2 pr-2 font-medium text-zinc-900">{r.label}</td>
      ...
      <p className="text-center text-[11px] text-zinc-500">
        Cifras gerenciales. No sustituyen cierre contable sin revisión del asesor. Comparativa: {data ? `${data.prevFrom} → ${data.prevTo}` : '—'}{' '}
        vs {data ? `${data.periodFrom} → ${data.periodTo}` : '—'}.
      </p>
    </div>
  );
}
