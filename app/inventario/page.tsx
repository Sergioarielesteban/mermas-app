...
export default function InventarioPage() {
  ...
  const catalogIdsInUse = useMemo(() => {
    const s = new Set<string>();
    for (const row of lines) {
      if (row.catalog_item_id) s.add(row.catalog_item_id);
    }
    return s;
  }, [lines]);
  ...
  const addFromCatalog = async (item: InventoryCatalogItem) => {
    if (!localId || !supabaseOk) return;
    const supabase = getSupabaseClient()!;
    const {
      data: { user },
  ...
  const saveLine = async (row: InventoryItem) => {
    if (!localId || !supabaseOk) return;
  ...
  return (
    <div className="space-y-5">
      ...
      <MermasStyleHero
        eyebrow="Chef-One"
        title="Inventario"
        description={`Stock y valor por artículo (${localLabel}). Añade líneas desde el catálogo común, ajusta cantidad y precio de valoración.`}
      />
      ...
      {loading ? (
        ...
        <>
            ...
            {lines.length === 0 ? (
              ...
              <ul className="space-y-2">
                {lines.map((row) => {
                  ...
                  const u = UNIT_SUFFIX[row.unit] ?? row.unit;
                  return (
                    <li
                      key={row.id}
                    ...
                    >
                      ...
                      {row.format_label ? (
                        <p className="mt-0.5 text-[11px] leading-snug text-zinc-500">{row.format_label}</p>
                      ) : null}
                      <div className="mt-2 flex flex-wrap items-end gap-2">
                        <label className="flex flex-col gap-0.5">
                          ...
                          <input
                            ...
                            className="h-9 w-[4.5rem] rounded-lg border border-zinc-200 px-2 text-sm font-semibold"
                          />
                        </label>
                        <label className="flex flex-col gap-0.5">
                          ...
                          <input
                            ...
                            className="h-9 w-[4.75rem] rounded-lg border border-zinc-200 px-2 text-sm font-semibold"
                          />
                        </label>
          ...
          <section>
            <h2 className="mb-2 text-base font-bold text-zinc-900">Catálogo (añadir a tu local)</h2>
            <input
              type="search"
              value={search}
                ...
                return (
                  ...
                  >
                    ...
                    <ul className="space-y-1 border-t border-zinc-100 px-2 py-2">
                      {items.map((it) => {
                        const has = catalogIdsInUse.has(it.id);
                        return (
                          <li
                            key={it.id}
                          ...
                          >
                            ...
                            {has ? (
                              <span className="text-[10px] font-bold uppercase text-emerald-700">En tu lista</span>
                            ) : (
                              <button
                                type="button"
                                disabled={disabled || busyId === it.id}
                                onClick={() => void addFromCatalog(it)}
                                className="inline-flex h-8 items-center gap-1 rounded-lg bg-zinc-900 px-2.5 text-[10px] font-bold text-white disabled:opacity-45"
                              >
